function Invoke-TestHarness
{
    ...
    Write-Verbose -Message 'Commencing all iSCSIDsc tests'

    $repoDir = Join-Path -Path $PSScriptRoot -ChildPath '..\' -Resolve

    $testCoverageFiles = @()
    Get-ChildItem -Path "$repoDir\modules\iSCSIDsc\DSCResources\**\*.psm1" -Recurse | ForEach-Object {
    ...
    Import-Module -Name "$repoDir\modules\iSCSIDsc\iSCSIDsc.psd1"
    $testsToRun = @()
